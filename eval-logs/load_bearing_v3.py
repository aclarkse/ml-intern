#!/usr/bin/env python3
"""
Load-Bearing Tokens in Chain-of-Thought Reasoning — v3
=======================================================
Stage-0 Pre-Registration:
  ...
  Measurable Criterion:
    ...
    - "Answer changed": Exact match of final answer (after #### or \boxed{})
    - H1 threshold: load-bearing ≥70% change vs. non-load-bearing <20%
    - F1 threshold: random perturbation ≥50% answer change rate

Model: Qwen/Qwen2.5-7B-Instruct (uses \boxed{} for final answers)
...
def extract_answer_from_model(text: str) -> Optional[str]:
    """Extract answer from model output — handles \boxed{} or #### format."""
    # Try \boxed{} first (model uses LaTeX box notation)
    match = re.search(r'\\boxed\{(\d+)\}', text)
    if match:
        return match.group(1)
    # Fall back to #### format (original dataset format)
    match = re.search(r'####\s*(\S+)', text)
    if match:
        return match.group(1)
    # Last resort: find the last number in text
    numbers = re.findall(r'\d+', text)
    return numbers[-1] if numbers else None

def extract_answer_from_dataset(answer_text: str) -> str:
    """Extract answer from dataset reference (#### format)."""
    match = re.search(r'####\s*(\S+)', answer_text)
    return match.group(1) if match else None

def generate_answer(question: str, cot_prefix: str = None, max_tokens: int = 250) -> str:
...
def find_positions_by_type(decoded: List[str]) -> dict:
    ...
    for i, t in enumerate(decoded):
        t_stripped = t.strip()
        if re.match(r'^[\+\-\*/=]$', t_stripped):
            positions['operator'].append(i)
        elif re.match(r'^\d+$', t_stripped):
            positions['number'].append(i)
        elif any(kw in t_stripped.lower() for kw in ['more', 'less', 'each', 'total', 'add', 'subtract', 'times', 'plus', 'minus', 'half', 'double', 'every', 'equals']):
...
def perturb_token(token_str: str, ptype: str) -> str:
    if ptype == 'operator':
        return {'+': '-', '-': '+', '*': '/', '/': '*', '=': 'X'}.get(token_str.strip(), token_str)
...
def run_contamination_check(problems: List[dict]) -> dict:
    ...
    for i, p in enumerate(problems[:5]):
        q = p['question']
        true_ans = extract_answer_from_dataset(p['answer'])
        
        real_out  = generate_answer(q)
        real_ans  = extract_answer_from_model(real_out)
        real_ok   = (real_ans == true_ans)
        
        rand_cot  = "Step 1: Analyze. Step 2: Compute. Step 3: Answer."
        rand_out  = generate_answer(q, rand_cot)
        rand_ans  = extract_answer_from_model(rand_out)
        rand_ok   = (rand_ans == true_ans)
        
        results['real'].append(real_ok)
        results['random_cot'].append(rand_ok)
        
        print(f"  [{i}] real={real_ok} (got '{real_ans}', expected '{true_ans}')", flush=True)
        print(f"      rand={rand_ok} (got '{rand_ans}')", flush=True)
...
def run_perturbation_exp(problems: List[dict], phase: str) -> dict:
    ...
    for i, p in enumerate(problems):
        ...
        # Extract just the reasoning part (before final answer)
        if '\\boxed{' in raw_out:
            cot = raw_out.split('\\boxed{')[0].strip()
        elif '####' in raw_out:
            cot = raw_out.split('####')[0].strip()
        else:
            # Try to split on "Therefore" or "So" before final answer
            parts = re.split(r'(?:Therefore|So,|In conclusion)', raw_out)
            cot = parts[0].strip() if len(parts) > 1 else raw_out[len(q):].strip()
        
        decoded  = tokenize_cot(cot)
        pos_dict = find_positions_by_type(decoded)
        orig_ans = extract_answer_from_model(raw_out)
        ...
        for ptype in ['operator', 'number', 'keyword', 'calc']:
            ...
            for pos in positions:
                ...
                pert_ans = extract_answer_from_model(pert_out)
        ...
        if len(decoded) > 5:
            ...
            changes  = 0
            for pos in rand_pos:
                all_tokens = tokenizer.encode(cot, add_special_tokens=False)
                # Replace with a different token
                replacement = random.choice([t for t in all_tokens if t != all_tokens[pos]])
                new_tokens  = all_tokens[:pos] + [replacement] + all_tokens[pos+1:]
                new_cot     = tokenizer.decode(new_tokens)
...
def analyze(disco: dict, val: dict, contam: dict) -> dict:
    ...
    if max_struct >= 0.7 and avg_random < 0.5:
        ...
        if ratio > 2:
            result['hypothesis_supported'] = True
            print("     (structured/random ratio suggests some structure effect)", flush=True)
